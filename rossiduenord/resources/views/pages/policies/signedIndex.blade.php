...
@section('content')
 ...
 <form action="{{route('policies.signed.store', $policy)}}" method="post" enctype="multipart/form-data">
     @csrf
     <div class="px-20 bg-white pb-5">
        <a href="{{route('policies.index',$policy->practice)}}" class="back"> <i class="fa-solid fa-arrow-left"></i> Torna indietro</a>
                ...
                <tbody id="table_ContentListFolder" class="text-center">
                    @forelse ($modifieds as $modified)
                        <tr>
                            ...
                            <td>{{$modified->name}}</td>
                            <td>
                                <a href="{{route('modified.download',$modified->id)}}" class="clickable"> Scarica <i class="fa-solid fa-file-arrow-down fa-1x"></i> </a>
                             </td>
                             <td>{{$modified->created_at ? $modified->created_at->format('d/m/y') : '' }}</td>
                             <td class="text-left" > <a href="{{route('modified.show', $modified)}}" class="btn btn-primary">Vedi</a> </td>
                        </tr>
                    @empty
