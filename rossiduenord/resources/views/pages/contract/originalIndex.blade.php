                        ...
                        <tbody id="table_ContentListFolder" class="text-center">
                            @forelse ($contracts as $contract)
                                <tr>
                                    <td>{{$contract->id}}</td>
                                    <td>{{$contract->name}}</td>
                                    <td> <a href="{{route('contract.download',$contract->id)}}" class="clickable"> Scarica <i class="fa-solid fa-file-arrow-down fa-1x"></i> </a> </td>
                                    <td> {{count($contract->signeds) > 0 ? 'Caricato' : 'Nessuna modifica'}}</td>
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{route('signed.index',$contract)}}" class="btn btn-primary px-3">Carica</a>
                                        </div>
                                    </td>
                                </tr>
                            @empty
