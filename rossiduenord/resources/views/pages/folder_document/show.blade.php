			...
			@foreach ($folder_documents as $folder_document)
				<div>
					<a href="{{ route('folderDocument.show', [$practice, $folder_document])}}">
						<button type="button"
						        class="folder mb-2 {{'folderDocument.show' && request()->route()->parameter('folder_document')->name == $folder_document->name ? 'active_folder' : ''}}">
						...
						<tr class=" row_folder {{request()->route()->parameter('sub_folder') ? request()->route()->parameter('sub_folder')->name == $sub_folder->name ? 'selected_folder' : '' : ''}}">
							<td class="text-left">
								<div class="d-flex align-items-center justify-content-around">
									<div class="d-flex align-items-center justify-content-center folder_document_status @if($sub_folder->assev_t_status == 0) not_viewed @elseif($sub_folder->assev_t_status == 1) not_approved @else approved @endif"
									     data-toggle="tooltip" data-placement="top"
									     title="@if($sub_folder->assev_t_status == 0) Non visualizzato @elseif($sub_folder->assev_t_status == 1) Non approvato @else Approvato @endif dall' Assev. T">
										T
									</div>
									<div class="d-flex align-items-center justify-content-center folder_document_status @if($sub_folder->assev_f_status == 0) not_viewed @elseif($sub_folder->assev_f_status == 1) not_approved @else approved @endif"
									     data-toggle="tooltip" data-placement="top"
									     title="@if($sub_folder->assev_f_status == 0) Non visualizzato @elseif($sub_folder->assev_f_status == 1) Non approvato @else Approvato @endif dall' Assev. F">
										F
									</div>
									<div class="d-flex align-items-center justify-content-center folder_document_status @if($sub_folder->bank_status == 0) not_viewed @elseif($sub_folder->bank_status == 1) not_approved @else approved @endif"
									     data-toggle="tooltip" data-placement="top"
									     title="@if($sub_folder->bank_status == 0) Non visualizzato @elseif($sub_folder->bank_status == 1) Non approvato @else Approvato @endif dalla Banca">
										B
									</div>
								</div>
							</td>
			...
			<div class="mt-3">
				@if($documents ?? '')
						...
						<div>
							...
							@if(auth()->user()->role->name === 'technical_asseverator' || auth()->user()->role->name === 'fiscal_asseverator' || auth()->user()->role->name === 'bank')
								...
								@if($current_sub_folder->$status == 2)
									<a href="{{ route('document.disapprove', [$practice, $current_sub_folder->folder_document, $current_sub_folder])}}">
					...
					<table class="table_bonus" style="width: 100%">
						...
						@forelse($documents as $document)
							<tr>
								...
								<td class="text-center">
									<a style="color: #61a4d7!important"
									   href="{{route('document.download',$document->id)}}">scarica</a>
								</td>
								<td class="d-flex">
									<button type="button" class="btn" data-toggle="modal"
									        data-target="#del{{$document->id}}">
										<img style="width: 17px;" src="{{ asset('/img/icon/icona_cancella.svg') }}"
										     alt="">
										<p class="m-0" style="color: #818387; font-size: 12px">Elimina</p>
									</button>

									<div class="modal fade" id="del{{$document->id}}" tabindex="-1" role="dialog"
									     aria-labelledby="exampleModalLabel" aria-hidden="true">
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												<div class="modal-header">
													<h5 class="modal-title" id="exampleModalLabel">Conferma elimina
														file</h5>
												...
												<div class="modal-footer">
													<button type="button" class="btn btn-secondary"
													        data-dismiss="modal">indietro
													</button>
													<form action="{{ Route('document.destroy', [$practice->id, $folder_document->id, $sub_folder->id, $document->id])}}"
													      method="POST">
														@csrf
														@method('DELETE')
					...
					<div class="d-flex mt-3 align-items-center">
						<a class="add-btn-custom" style="margin-left: 0" href="#FormDocument" data-toggle="collapse"
						   role="button" aria-expanded="false" aria-controls="collapseExample">+</a>
						<h4 class="mt-2">Inserisci documento</h4>
					</div>

					<form action="{{ route('document.store', [$practice->id, $folder_document->id, $sub_folder->id] )}}"
					      method="POST" enctype="multipart/form-data" class="collapse" id="FormDocument"
						...
						<div class="box_input">
							<div class="row_input">
								...
								<label for="folder_id" class="text" style="width: 70%">
									Cartella
									<select style="height: 47px!important; text-align:left;" class="form-control"
									        name="sub_folder_id" id="sub_folder_id">
								...
								<label for="allega" style="width: 30%">
									Allega
									<input style="height: 47px!important" class="form-control" type="file" name="allega"
									       id="allega">
								</label>
							</div>
							<div class="row_input">
								<label style="width: 100%;" for="note">
									Note
									<input style="height: 47px!important; width:100%; text-align:left"
							...
							<div class="row_input">
								...
								<label class="checkbox-wrapper d-flex">
									<input type="checkbox" name="type" id="type"
									       value="Documento che necessita di approvazione speciale">
									<span class="checkmark"></span>
