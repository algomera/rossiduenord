...
<div class="form-group">
    ...
    <div class="position-relative">
        <div class="d-flex align-items-center">
            @if($subject[$name])
            <div class="viewAnagrafica mr-2 btn bg-blue white" data-toggle="modal" data-target="#anagrafiche-modal" data-id="{{$subject[$name]}}">
        ...
        <select onchange="setSubject({{$subject->practice->id}}, this.name, this.value)" id="{{$name}}" name="{{$name}}" class="col-md form-control credit-input @error($name) is-invalid @enderror" style="height: 40px; border-radius: 2px; border: 1px solid rgb(219, 220, 219); background-color: rgb(242, 242, 242);">
            ...
            @foreach($items as $item)
                <option value="{{ $item->id }}" {{ $subject[$name] === $item->id ? 'selected' : '' }}>{{ $item->company_name }} @if($name === 'consultant' && $item->consultant_type)({{ $item->consultant_type }})@endif</option>
            @endforeach
        </select>
        </div>
    </div>
