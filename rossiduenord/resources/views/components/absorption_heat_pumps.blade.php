...
<div class="mt-5">
    <div class="d-flex align-items-center mb-3">
        ...
        <div class="add-btn-custom" onclick="addAbsorptionHeatPump(event)">+</div>
        @if($items->count() > 0)
                ...
                <div class="box_input" data-id="absorption_heat_pump-{{$practice->id}}-{{$item->id}}">
                    ...
                    <div class="row_input">
                        ...
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_sostituito]">
                            Tipo sostituito
                            <select name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_sostituito]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_sostituito]">
                        ...
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][p_nom_sostituito]">
                            P. nom. sostituito
                            <input class="input_small" type="number" name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][p_nom_sostituito]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][p_nom_sostituito]" value="{{ $item->p_nom_sostituito }}">
                            kW
                        </label>
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_di_pdc]">
                            Tipo di PDC
                            <select name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_di_pdc]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_di_pdc]">
                    ...
                    <div class="row_input">
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_roof_top]" class="checkbox-wrapper d-flex">
                            <input type="checkbox" {{ $item->tipo_roof_top ? 'checked' : '' }} name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_roof_top]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][tipo_roof_top]" value="true">
                            <span class="checkmark"></span>
                            Tipo Roof Top
                        </label>
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][potenza_nominale]">
                            Potenza Nominale
                            <input class="input_small" type="number" name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][potenza_nominale]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][potenza_nominale]" value="{{ $item->potenza_nominale }}">
                            kW
                        </label>
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][gueh]">
                            GUEh
                            <input class="input_small" type="number" name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][gueh]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][gueh]" value="{{ $item->gueh }}">
                        </label>
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][reversibile]" class="checkbox-wrapper d-flex">
                            <input type="checkbox" {{ $item->reversibile ? 'checked' : '' }} name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][reversibile]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][reversibile]" value="true">
                            <span class="checkmark"></span>
                            E' reversibile
                        </label>
                        <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][guec]">
                            GUEc
                            <input class="input_small" type="number" name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][guec]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][guec]" value="{{ $item->guec }}">
                        </label>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="row_input">
                            <label for="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][sup_riscaldata_dalla_pdc]">
                                Sup. riscaldata dalla PDC
                                <input class="input_small" type="number" name="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][sup_riscaldata_dalla_pdc]" id="absorption_heat_pumps[{{$practice->id}}-{{$item->id}}][sup_riscaldata_dalla_pdc]" value="{{ $item->sup_riscaldata_dalla_pdc }}">
                                m²
                            </label>
                        </div>
                        <div onclick="deleteAbsorptionHeatPump({{$practice->id}}, {{$item->id}})" style="border: none; background-color: transparent;" class="d-flex flex-column align-items-center justify-content-center mr-3">
