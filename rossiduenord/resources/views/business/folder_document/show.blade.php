    ...
    <div class="d-flex pb-20">
        ...
        <div class="px-20 pb-20" style="width: 20%; border-right: 1px solid lightgrey;">
            ...
            @foreach ($folder_documents as $folder_document)
                <div>
                    <a href="{{ route('business.folderDocument_show', [$practice->id, $folder_document->id])}}">
                        <button type="button" class="add-button mb-2 {{'business.folderDocument_show' && request()->route()->parameter('folder_document')->name == $folder_document->name ? 'active_folder' : ''}}">
                ...
                <table class="table_bonus" style="width: 100%">
                    <thead>
                        <tr>
                            ...
                            <td style="width:15%;"><b>Ruolo</b></td>
                    ...
                    <tbody>
                        @foreach($folders as $folder)
                            <tr>
                                ...
                                <td class="text-left">{{$folder->status}}</td>
                                <td class="text-left">{{$folder->name}}</td>
                                <td class="text-left">{{$folder->role}}</td>
                                <td class="text-center">
{{--                                      <a href="{{ route('business.document_show', [$practice->id, $folder_document->id, $documents->sub_folder_id])}}">
 ...
 --}}                                </td>
            ...
            <div>
                <div class="d-flex">
                    <h3>Lista documenti Cartella N.{{$folder->id}}</h3>
                    ...
                    <div class="box_input">
                        <div class="row_input">
                            ...
                            <label for="folder_id" class="text">
                                Cartella
                                <select style="height: 47px!important" class="form-control" name="sub_folder_id" id="sub_folder_id">
                                    <option selected disabled value="">Seleziona cartella</option>
                                    @foreach($folders as $folder)
                                        <option {{request()->route()->parameter('folder_document')->name == $folder_document->name ? 'selected' : ''}} value="{{$folder->id}}">{{$folder->name}}</option>
                                    @endforeach
                                </select>
                            </label>
                            <label for="allega">
                ...
                <table class="table_bonus" style="width: 100%">
                    <thead>
                        <tr>
                            ...
                            <td style="width:50%;"><b>Note documento</b></td>
