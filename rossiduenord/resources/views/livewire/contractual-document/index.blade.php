		...
		<div>
			<div class="sm:hidden">
				...
				<select wire:model="selected" wire:ignore id="tabs" name="tabs"
					...
					<option @if($tab === $k) selected @endif value="{{ $tab->id }}">{{ $tab->user_data->name }}</option>
					@endforeach
				</select>
			</div>
				...
				<tr wire:key="{{ $loop->index }}">
					...
					<x-table.td>
						<div class="flex items-center space-x-5">
							...
							{{--									@isset($uploaded_contractual_document[$file_document[$loop->index]->id])--}}
							{{--										@if($uploaded_contractual_document[$file_document[$loop->index]->id])--}}
							{{--											<x-button type="button"--}}
							{{--											          wire:click="upload({{ $file_document[$loop->index]->id }})"--}}
							{{--											          size="xs"--}}
							...
							{{--										@endif--}}
							{{--									@else--}}
							{{--										<label class="inline-flex items-center border border-transparent font-medium shadow-sm text-white @if($file_document[$loop->index]->uploaded_path) bg-green-600 hover:bg-green-700 focus:ring-green-500 @else bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 @endif focus:outline-none focus:ring-2 focus:ring-offset-2 px-2.5 py-1.5 text-xs rounded cursor-pointer disabled:opacity-25 transition">--}}
							{{--											<input wire:model="uploaded_contractual_document.{{$file_document[$loop->index]->id}}"--}}
							{{--											       type="file" name="file_document"--}}
							{{--											       id="file_document"--}}
							{{--											       class="sr-only"/>--}}
							{{--											<x-icon wire:loading.remove--}}
							{{--											        wire:target="uploaded_contractual_document.{{$file_document[$loop->index]->id}}"--}}
							...
							{{--									                 wire:click="delete({{ $file_document[$loop->index]->id }})"--}}
							{{--									                 size="xs">--}}
