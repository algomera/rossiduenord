...
@section('content')
 ...
 <form action="{{route('asseverator.signed.store',$contract)}}" method="post" enctype="multipart/form-data">
     @csrf
     <div class="px-20 pb-5 bg-white">
        <a href="{{route('asseverator.contracts.index', $contract->practice)}}" class="back"> <i class="fa-solid fa-arrow-left"></i> Torna indietro</a>
                ...
                <tbody id="table_ContentListFolder" class="text-center">
                    @forelse ($signeds as $signed)
                        <tr>
                            ...
                            <td>{{$signed->name}}</td>
                            <td>
                                <a href="{{route('asseverator.signed.download',$signed->id)}}" class="clickable"> Scarica <i class="fa-solid fa-file-arrow-down fa-1x"></i> </a>
                             </td>
                             <td>{{$signed->created_at ? $signed->created_at->format('d/m/y') : '' }}</td>
                             <td class="text-left" > <a href="{{route('asseverator.signed.show', $signed)}}" class="btn btn-primary">Vedi</a> </td>
                        </tr>
                    @empty
