...
@section('content')
    ...
    @include('asseverator.layouts.partials.nav_superbonus')
            <form action="{{ route('asseverator.update_towed_vertical_wall', ['practice' => $practice]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="d-flex ov-x-none" style="font-weight: 500">
                    <div class="px-20 border-right" style="width: 20%">{{-- column left --}}
                        <a href="{{ route('asseverator.towed_intervention', ['practice'=>$practice->id, 'condomino'=>'common', 'type'=>'PV'])}}" class="black">
                        ...
                        <a href="{{ route('asseverator.towed_intervention', ['practice'=>$practice->id, 'condomino'=>$condomino->id, 'type'=>'PV'])}}">
...
{{--     @push('scripts')
        @include('asseverator.scripts.select_country')
    @endpush
 --}}@endsection
...
/*             axios.post(`/asseverator/show_condomino_data/${id}`)
                .then(() => {
 ...
 */        }
        function saveCondominoChanges(pid, id) {
            ...
            axios.post(`/asseverator/save_condomino_data/${id}`, {
